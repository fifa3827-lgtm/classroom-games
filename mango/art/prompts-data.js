...
/* ───────────────────────── 2. 사건 1 인물 ───────────────────────── */
{
  ...
  items: [
    ...
    { file:"doto-STEP1.png", label:"도토 · 1단계 — 기본 한 장 (첨부 없이)",
      note:"★ 꼬리는 반드시 또렷한 갈색, 그리고 고리 무늬가 없어야 한다. 도토를 범인에서 지우는 근거가 「뒷문으로 나간 건 회색 꼬리였는데 도토는 갈색이다」 하나뿐이다. 꼬리가 회색으로 읽히거나 너구리처럼 고리가 있으면 소거가 무너지므로 다시 뽑는다. 실루엣 규칙(바이블 9.5): 너구리들은 둥근 항아리 몸인데 도토는 길고 좁은 몸 + 머리보다 큰 꼬리라, 검은 실루엣으로 바꿔도 구별돼야 한다.",
      p:"An anthropomorphic squirrel as a grumpy tree-house landlord, one single front-facing bust portrait. Tall narrow vertical body, very large fluffy tail curving up behind the head — the tail is the biggest shape in the picture. The tail and body fur are a warm REDDISH-BROWN, clearly BROWN and absolutely NOT grey, and the tail is ONE SOLID BROWN with NO rings, NO stripes and NO bands of any kind. Cream chest, tufted ear tips, holding ONE acorn in both front paws at chest height. Sleepy half-lidded eyes and a slight frown — grumpy and just woken from a nap. He is NOT a detective — no vest, no bow tie, no bag, no magnifying glass, no hat. "+STYLE.cel },

    { file:"doto-SHEET.png", label:"도토 · 2단계 — 표정 3칸 (★ 승인된 도토 1단계 그림을 첨부할 것)",
      note:"첨부는 반드시 도토 자신의 1단계 그림. 사건 1에서 도토는 반박당할 진술이 없어 기본 표정만 쓰지만, 뒤 사건에서 용의자가 되면 놀람·당황이 필요하고 그때 다시 뽑으면 얼굴이 달라진다. 분노 칸(`doto-ang`)은 게임이 쓰는 곳이 없어 만들지 않는다. 받은 파일은 art/slice-sheet.py <파일> doto def,sp,fl 로 자른다.",
      p:"Using the attached character as the exact reference, draw the SAME squirrel three times in one image, side by side in a single row, in this order: (1) sleepy and grumpy, half-lidded eyes, slight frown, looking straight ahead; (2) startled awake — eyes suddenly wide open and round, tail fur bristling outward, mouth open in a small gasp; (3) taken aback — eyes blinking wide, mouth a small wavy line, one paw scratching the back of the head. Keep his face, body, the warm reddish-brown SOLID tail with NO rings, the cream chest and the single acorn exactly as in the attached image in all three panels — same size, same colors, same line weight. Leave a clear empty gap between the panels — the figures must NOT touch or overlap each other. Plain flat cream #fbf7ee background, no panel borders, no frames, no text, no labels. "+STYLE.cel },
